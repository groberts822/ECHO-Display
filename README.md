...
In addition, I used the provided STEP files to make the 3D viewer. Just a note, the DIsplay does not have headers in this 3D viewer, because they are specially spaced pins (3mm apart not 2.54mm), so I will have to use the header pins which come with the display itself. I also made sure to adjust the footprint so that it was actually 3mm apart and would work. Below is the 3D model: 
